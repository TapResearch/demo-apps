...
class TapResearchBasicsController: UIViewController {
        ...
        didSet {
            DispatchQueue.main.async(execute: { () -> Void in
                if self.surveyWallHasSurveys {
                    self.showSurveyWallPreviewButton.setTitle(
                        "Show Survey Wall Preview", for: .normal)
                } else {
                    self.showSurveyWallPreviewButton.setTitle(
                        "Has surveys for Survey Wall Preview?", for: .normal)
                }
            })
        }
    }
    ...
    override func viewDidLoad() {
        ...
        TapResearch.initialize(
            withAPIToken: apiToken, userIdentifier: userIdentifier, sdkDelegate: self
        ) { (error: NSError?) in
            if let error {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == TapResearchBasicsController.showSurveyWallPreviewSegue {
            if let vc: SurveyWallPreviewController = segue.destination
                as? SurveyWallPreviewController
            {
    ...
    @IBAction func showSurveyWallPreviewButtonTapped() {
        ...
        if surveyWallHasSurveys {
            performSegue(
                withIdentifier: TapResearchBasicsController.showSurveyWallPreviewSegue,
                sender: surveyWallPreviewPlacement)
            surveyWallHasSurveys = false
        } else {
            let hasSurveys: Bool = TapResearch.hasSurveys(for: surveyWallPreviewPlacement) {
                (error: NSError?) in
