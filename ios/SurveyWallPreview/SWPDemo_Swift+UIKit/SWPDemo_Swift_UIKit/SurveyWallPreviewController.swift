...
class SurveyWallPreviewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if TapResearch.canShowSurvey(surveyId: surveyId, forPlacementTag: placementTag) {
            TapResearch.showSurvey(surveyId: surveyId, placementTag: placementTag, delegate: self) {
                (error: NSError?) in
                if let error {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var info: String =
            "\(survey.lengthInMinutes) \(survey.lengthInMinutes == 1 ? "minute" : "minutes"), \(survey.rewardAmount) \(survey.currencyName)"
        if survey.isSale {
