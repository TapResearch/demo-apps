...
@interface SurveyWallPreviewController () <UITableViewDelegate, UITableViewDataSource, TapResearchContentDelegate, TapResearchSurveysDelegate>
...
@implementation SurveyWallPreviewController

- (void)viewDidLoad {
	[super viewDidLoad];

	[TapResearch setSurveysDelegate:self];
	self.surveys = [[NSMutableArray alloc] initWithArray:[TapResearch getSurveysFor:self.surveysPlacement errorHandler:^(NSError * _Nullable error) {
	}]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSString *title = [NSString stringWithFormat:@"%@ %@", self.surveysPlacement, survey.surveyIdentifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([TapResearch canShowSurveyWithSurveyId:surveyId forPlacementTag:self.surveysPlacement]) {
		...
		[TapResearch showSurveyWithSurveyId:surveyId placementTag:self.surveysPlacement delegate:self errorHandler:^(NSError * _Nullable error) {
			if (error) {
