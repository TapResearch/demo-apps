...
namespace TapResearch.Editor
{
    public class BuildPostprocessoriOS
    {
        ...
        private static string AppAssetsPath => Path.Combine(Application.dataPath, "TapResearchSDK", "Plugins", "Templates", "iOS");
        ...
        private static void HandleiOSPostBuild(string buildPath)
        {
            ...
            project.SetBuildProperty(mainProjectGuid, "ALWAYS_EMBED_SWIFT_STANDARD_LIBRARIES", "NO");
        ...
        {            
            ...
            Directory.CreateDirectory(Path.Combine(buildPath, "TapResearch"));

            foreach (var framework in requiredFrameworks)
            {
                // string sourcePath = Path.Combine(AppAssetsPath, "Frameworks", framework);
                // var destPath = Path.Combine(buildPath, "TapResearch", framework);
                ...
                var frameworkGuid = project.AddFile(destPath, Path.Combine("TapResearch", framework));
