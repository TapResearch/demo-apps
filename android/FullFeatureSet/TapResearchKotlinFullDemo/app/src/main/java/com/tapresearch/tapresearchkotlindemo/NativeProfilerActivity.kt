...
class NativeProfilerActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val answers = remember { mutableStateMapOf<Int, QualificationAnswerState>() }
                            ...
                            Column(
                                ...
                                if (response?.error != null) {
                                    ...
                                    if (response.isProfiled == true) {
                                        ...
                                        Text(text = "The following qualification question(s) are required:")
                                    }

                                    // Qualifications that need to be answered
                                    response.qualifications?.forEach { qual ->
                                        QualificationItem(
                                            qualification = qual,
    ...
    private fun submitAnswers(answers: Map<Int, QualificationAnswerState>) {
...
@Composable
fun QualificationItem(
    qualification: TRQualification,
    state: QualificationAnswerState,
    onStateChanged: (QualificationAnswerState) -> Unit
) {
            ...
            "single_select" -> {
                qualification.qualificationAnswers?.forEach { answer ->
                    Row(
                        ...
                        modifier = Modifier
                            .fillMaxWidth()
                            .clickable { onStateChanged(QualificationAnswerState(listOf(answer.preCode ?: ""))) }
                    ) {
                        RadioButton(
                            selected = state.values.contains(answer.preCode),
                            onClick = { onStateChanged(QualificationAnswerState(listOf(answer.preCode ?: ""))) }
                        )
